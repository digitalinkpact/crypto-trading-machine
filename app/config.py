...
SYMBOLS: tuple[str, ...] = (
    ...
    "OPUSDT", "SHIBUSDT", "SUIUSDT", "RENDERUSDT", "AAVEUSDT",
)
...
class Settings(BaseSettings):
    ...
    # Include the LLM reasoner agent inside the autopilot's fast trading loop.
    # Off by default: LLM calls are slow, non-deterministic, and cost money.
    # When False, LLM signals still refresh via the scheduled `llm_pass` job.
    ...
    # ── Feature flags (default OFF so half-integrated features stay dormant) ──
    live_mode: bool = False
    live_price_enabled: bool = False
    live_price_max_age_seconds: int = 30
    derivatives_data_enabled: bool = False
    onchain_enabled: bool = False
    onchain_cache_ttl_seconds: int = 300
    onchain_timeout_seconds: int = 10
    onchain_inflow_spike_z: float = 3.0
    ...
    orderbook_near_pct: float = 0.005
    min_depth_trade_multiple: float = 3.0
    risk_loop_in_process_enabled: bool = False
    risk_manager_loop_seconds: int = 60
    ml_gate_enabled: bool = False
    ml_gate_threshold: float = 0.55
    ml_gate_max_model_age_hours: float = 48.0
    ml_label_slippage_pct: float = 0.001
    online_regime_min_samples: int = 100
    dynamic_threshold_enabled: bool = False
    dynamic_threshold_max_delta: float = 0.1
    entry_strategy: str = "aggregator"

    # ── Risk / spread / drawdown guards ──
    risk_per_trade_pct: float = Field(0.01, ge=0.0, le=0.5)
    daily_loss_limit_pct: float = Field(0.05, ge=0.0, le=1.0)
    max_spread_pct: float = Field(0.01, ge=0.0, le=0.5)
    avg_spread_pct: float = Field(0.005, ge=0.0, le=0.5)
    max_dip_extension_pct: float = Field(0.10, ge=0.0, le=1.0)
    funding_min_pct: float = Field(-1.0, ge=-1.0, le=1.0)     # -1 disables the gate
    trailing_activation_pct: float = Field(0.01, ge=0.0, le=0.5)
    loss_streak_pause_count: int = Field(4, ge=1, le=100)
    loss_streak_pause_minutes: int = Field(60, ge=0, le=1440)

    # ── Emergency halt / health monitor ──
    emergency_halt_max_failures: int = 5
    emergency_halt_auto_clear_cycles: int = 10
    health_tick_stale_seconds: int = 900
    health_latency_warn_seconds: float = 5.0
    health_cpu_warn_pct: float = 90.0
    health_memory_rss_warn_mb: int = 1024
    health_order_failure_max: int = 5
    health_order_failure_lookback_minutes: int = 30
    health_duplicate_order_window_seconds: int = 5

    # ── Auth / session (dashboard) ──
    auth_session_hours: int = 12
    auth_remember_days: int = 30
    auth_token_minutes: int = 15
    auth_max_failed: int = 5
    auth_lockout_minutes: int = 15
    auth_ip_allowlist: str = ""
    force_https: bool = False

    # ── SMTP alerts (blank disables) ──
    smtp_host: str = ""
    smtp_port: int = 587
    smtp_user: str = ""
    ...
    llm_web_cache_ttl_seconds: int = 300
    llm_web_timeout_seconds: int = 15

    # ── Mean-reversion exit (agents/mean_reversion.py flags) ──
    mean_reversion_exit_enabled: bool = False
    mean_reversion_exit_defer_to_risk_ladder: bool = True
    mean_reversion_exit_min_pnl_pct: float = 0.01
    mean_reversion_exit_rsi: int = 70
    ...
    oversold_bounce_rsi_max: int = 30
    oversold_bounce_bb_multiplier: float = 1.0
    oversold_bounce_min_bounce_pct: float = 0.005

    # ── Profit-stream / news gating ──
    profitstream_score_threshold: float = 0.6
    profitstream_low_volume_quote_min: float = 100_000.0
    profitstream_news_buffer_minutes: int = 30
