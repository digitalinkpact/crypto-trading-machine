...
class Settings(BaseSettings):
    ...
    # Raised 0.60->0.95 so the 50-position count cap can actually bind: at ~1.9%
    # per position, 50 positions need ~95% of equity deployed in non-USDT.
    max_long_exposure_pct: float = Field(0.95, ge=0.0, le=1.0)   # ≤ 95% of equity in non-USDT
    # Absolute floor on order notional (USDT). Trades sized below this are lifted
    # to this value if cash allows, else skipped. Binance.US MIN_NOTIONAL is $10.
    min_order_usdt: float = Field(10.0, ge=1.0, le=10000.0)
    # Confidence-scaled sizing — high-conviction signals get a bigger slice.
    # Size multiplier ramps linearly from confidence_sizing_floor (at the entry
    # confidence bar) to 1.0 (at confidence == 1.0).
    confidence_sizing_enabled: bool = True
    confidence_sizing_floor: float = Field(0.5, ge=0.0, le=1.0)
    ...
    buy_cooldown_minutes: int = Field(90, ge=0, le=1440)         # raised 30->90 to cut buy->stop->rebuy fee churn
